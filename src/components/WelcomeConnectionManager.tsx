...
export const WelcomeConnectionManager = () => {
  ...
  return (
    <div className="flex-1 bg-background flex flex-col min-w-0">
      {/* Header with Search */}
      <div className="h-14 flex items-center px-6 border-b border-border gap-4 bg-sidebar">
        <div className="flex-1 relative group">
          ...
          <input 
            ...
            className="w-full h-8 bg-surface border border-border rounded-md pl-9 pr-3 text-[12px] text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent/40 focus:bg-sidebar transition-all"
          />
        </div>
        <button 
          onClick={() => setShowConnectionModal(true)}
          className="p-1.5 hover:bg-hover rounded-md text-text-muted hover:text-text-primary transition-colors"
        >
          <Plus size={16} />
        </button>
      </div>
        ...
        {filteredConnections.length > 0 ? (
          filteredConnections.map((conn) => (
            <div 
              ...
              className="flex items-center gap-4 px-4 py-3 rounded-lg hover:bg-hover cursor-default border border-transparent hover:border-border transition-all group"
            >
              ...
              <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                <button 
                  ...
                  className="p-1 text-text-muted hover:text-text-primary rounded hover:bg-surface-elevated"
                >
                ...
                <button 
                  ...
                  className="p-1 px-2 text-[10px] bg-surface text-text-secondary hover:text-text-primary rounded border border-border hover:bg-surface-elevated flex items-center gap-1.5 disabled:opacity-50 transition-colors"
                >
      ...
      {passwordPrompt.visible && passwordPrompt.connection && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center animate-in fade-in duration-200">
          <div className="bg-sidebar border border-border rounded-lg shadow-2xl w-[380px] animate-in zoom-in-95 duration-200">
            {/* Header */}
            ...
            <div className="p-4 space-y-4">
              ...
              <input 
                ...
                className="w-full bg-surface border border-border rounded px-3 py-2 text-sm text-text-primary placeholder:text-text-muted focus:border-accent focus:outline-none"
              />
            </div>
            ...
            <div className="p-4 bg-surface border-t border-border flex justify-end gap-3">
              <button 
                onClick={() => setPasswordPrompt({ visible: false, connection: null, password: '' })}
                className="px-4 py-1.5 text-xs text-text-secondary hover:bg-hover hover:text-text-primary rounded transition-colors"
              >
                Cancel
              </button>
              <button 
      ...
      {contextMenu.visible && (
        <div 
          className="fixed z-50 bg-sidebar border border-border rounded-md shadow-xl py-1 min-w-[150px] animate-in fade-in zoom-in-95 duration-100"
          style={{ left: contextMenu.x, top: contextMenu.y }}
        >
          <button 
            onClick={handleEdit}
            className="w-full px-3 py-1.5 text-left text-[12px] text-text-primary hover:bg-hover flex items-center gap-2"
          >
            <Pencil size={12} /> Edit
          </button>
          ...
          <button 
            ...
            className="w-full px-3 py-1.5 text-left text-[12px] text-red-400 hover:bg-hover flex items-center gap-2"
          >
      ...
      {/* Footer Actions */}
      <div className="p-6 border-t border-border flex items-center gap-6 bg-sidebar">
        <button 
          onClick={() => setShowConnectionModal(true)}
          className="flex items-center gap-2 text-[12px] text-text-muted hover:text-accent transition-colors group"
        >
          <div className="w-5 h-5 rounded bg-surface flex items-center justify-center group-hover:bg-accent/20 group-hover:text-accent transition-all">
            <Plus size={12} />
          </div>
