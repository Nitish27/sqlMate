...
use tauri::State;
use std::sync::Arc;
use crate::core::{AppState, ConnectionConfig, QueryResult, TableMetadata, FilterConfig, SidebarItem, SidebarItemType};
...
async fn connect(
    ...
    state.driver_registry.connect(config, password).await.map_err(|e| e.to_string())
}
...
async fn test_connection(
    ...
    state.driver_registry.test_connection(config, password).await.map_err(|e| e.to_string())
}
...
async fn create_database(
    ...
    QueryEngine::create_database(&state.driver_registry, &connection_id, &db_name).await
...
async fn get_databases(
    ...
    QueryEngine::get_databases(&state.driver_registry, &connection_id).await
        .map_err(|e| e.to_string())
}
...
async fn switch_database(
    ...
    state.driver_registry.switch_database(&connection_id, &db_name).await
        .map_err(|e| e.to_string())
}
...
async fn get_tables(
    ...
    QueryEngine::get_tables(&state.driver_registry, &connection_id).await
        .map_err(|e| e.to_string())
}
...
async fn get_sidebar_items(
    ...
    QueryEngine::get_sidebar_items(&state.driver_registry, &connection_id).await
        .map_err(|e| e.to_string())
}
...
async fn get_table_data(
    ...
    QueryEngine::get_table_data(&state.driver_registry, &connection_id, &table_name, limit, offset, filters, sort_column, sort_direction).await
        .map_err(|e| e.to_string())
}
...
async fn get_table_count(
    ...
    let filters = filters.unwrap_or_default();
    QueryEngine::get_table_count(&state.driver_registry, &connection_id, &table_name, filters).await
        .map_err(|e| e.to_string())
}
...
async fn get_table_metadata(
    ...
    QueryEngine::get_table_metadata(&state.driver_registry, &connection_id, &table_name).await
        .map_err(|e| e.to_string())
}
...
async fn get_table_structure(
    ...
    QueryEngine::get_table_structure(&state.driver_registry, &connection_id, &table_name).await
        .map_err(|e| e.to_string())
}
...
async fn execute_mutations(
    ...
    QueryEngine::execute_mutations(&state.driver_registry, &connection_id, statements).await
        .map_err(|e| e.to_string())
}
...
async fn export_table_data(
    ...
    let filters = filters.unwrap_or_default();
    QueryEngine::export_table_data(&state.driver_registry, &connection_id, &table_name, filters, sort_column, sort_direction, &format, &file_path).await
        .map_err(|e| e.to_string())
}
...
async fn text_to_sql(
    ...
    // Detect database type via DriverRegistry
    let db_type = {
        let dt = state.driver_registry.get_db_type_str(&connection_id).await;
        match dt {
            Some("postgres") => "PostgreSQL".to_string(),
            Some("mysql") => "MySQL".to_string(),
            Some("sqlite") => "SQLite".to_string(),
            _ => return Err("Connection not found".to_string()),
        }
    };

    // Fetch all tables/views from sidebar items
    let items = QueryEngine::get_sidebar_items(&state.driver_registry, &connection_id)
        .await
    ...
    for table_name in &table_names {
        match QueryEngine::get_table_structure(&state.driver_registry, &connection_id, table_name).await {
            Ok(structure) => {
